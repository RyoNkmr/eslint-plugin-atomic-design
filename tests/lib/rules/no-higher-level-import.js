...
const getFilePath = (...relativePath) => require('path').join(process.cwd(), 'tests', 'files', ...relativePath);
...
ruleTester.run('no-higher-level-import', rule, {
  valid: [
    ...
    // aliases
    ...spec('components/molecules/Component.js', '~/atoms/Component'),
    ...spec('components/molecules/Component.js', '@/components/atoms/Component'),

    // node_modules resolution
    ...spec('components/organisms/Component.js', 'component_library/molecules/Component'),
    // they're ignored by this rule as deafult
    ...spec('components/molecules/Component.js', 'component_library/molecules/Component'),
    ...
    // custom path parser
    ...spec('routes/pages/Component.js', '../../components/organisms/Component.js', {
      options: [{ pathPatterns: ['components\/(\\w+)\/', 'routes\/(\\w+)\/'] }],
    }),
    // passing through
    ...spec('routes/pages/Component.js', '../../components/concretes/Component.js', {
      options: [{ pathPatterns: ['components\/(\\w+)\/', 'routes\/(\\w+)\/'] }],
      errors: ['Do not require molecules from molecules. Molecules can contain only atoms.'],
    }),
  ],
  ...
  invalid: [
    ...
    ...spec('components/molecules/Component.js', '../organisms/Component', {
      errors: ['Do not require organisms from molecules. Molecules can contain only atoms.'],
    }),

    // same level import
    ...spec('components/atoms/Component.js', './Other', {
      errors: ['Do not require atoms from atoms. Atoms cannot require any other components.']
    }),
    ...
    ...spec('components/molecules/Component.js', '../concretes/Component.js', {
      options: [{ levels: ['atoms', ['molecules', 'concretes'], 'organisms'], }],
      errors: ['Do not require concretes from molecules. Molecules can contain only atoms.'],
    }),
    ...
    ...spec('routes/pages/Index.js', './About.js', {
      ...
      errors: ['Do not require pages from pages. Pages can contain atoms, molecules, organisms and templates.'],
    }),
  ],
});
